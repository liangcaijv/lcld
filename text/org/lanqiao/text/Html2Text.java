...
public class Html2Text {
	public static void main(String[] args) {
		String content = html2text("&#39;<pre>1.</pre><pre>2.</pre>");
		...
		if (c == '<') {
			...
			if (s.startsWith("<!--")) { // this is a comment!
				...
				if (end_comment_index == (-1)) {
					...
					return new Token(Token.TOKEN_COMMENT, data, start, data.length,
					    previousIsPre);
				} else
			...
			if (s_lowerCase.startsWith("<script")) { // this is a script:
				...
				if (end_script_index == (-1))
					...
					return new Token(Token.TOKEN_SCRIPT, data, start, data.length,
					    previousIsPre);
				else
					return new Token(Token.TOKEN_SCRIPT, data, start,
					    end_script_index + 9, previousIsPre);
			} else { // this is a tag:
				return new Token(Token.TOKEN_TAG, data, start, start + s.length(),
				    previousIsPre);
			}
		}
		// this is a text:
