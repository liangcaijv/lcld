 ...
 * 门栓：latch（CountDownLatch），多个线程各自执行，但是等到某个事件发生（归零），调用了await的控制线程取消阻塞，countdown方法并不阻塞
 ...
 * CycliBarrier（循环栅栏）：线程在栅栏前排队，**每个**都调用await方法，所有的线程都调用过await之后，阻塞状态全部解除
——与latch不同的是：latch阻塞控制线程，barrier阻塞工作线程本身
 * 
